...
var Storage = {
  write: function write(file, instance) {}
};
...
var SequelizeField = function () {
  ...
  function SequelizeField(_ref) {
    ...
    var groupByAttribute = _ref.groupByAttribute;
    ...
    this._destroyFileHook = function (instance, options) {
      ...
      if (_this._SIZES) {
        _this._forEachSize(_this._SIZES, function (size, name, options) {
          var pathForSize = pathWithSize(original, name);
          return _fs2.default.unlink(pathForSize, function (e) {});
        });
      }

      _fs2.default.unlink(original, function (e) {});
    };
    ...
    this._addHooksTo = function (Model) {
      ...
      var MODEL_FOLDER = "" + (0, _pluralize2.default)(Model.name.toLowerCase());

      if (_this._GROUP_BY_ATTRIBUTE) {
        MODEL_FOLDER += "/" + (0, _pluralize2.default)(_VIRTUAL_ATTRIBUTE_NAME.toLowerCase());
      }

      _this._MODEL_PATH = _this._BASE_PATH + "/" + MODEL_FOLDER;
    ...
    validateType({ folderKey: folderKey }, 'String | Undefined | Null');
    validateType({ groupByAttribute: groupByAttribute }, 'Boolean | Undefined');
  ...
  _createClass(SequelizeField, [{
    ...
    value: function _setFile(instance, options, afterCreate) {
      ...
      if (typeOf(file) === 'Object' && typeof file.mimetype === 'string' && typeof file.path === 'string') {
        ...
        var url = file;
        var filename = this._getFileNameForMoving.bind(this)(instance, url);
    ...
    key: "_getInstancePath",
    value: function _getInstancePath(instance) {
      if (!this._FOLDER_KEY) return this._MODEL_PATH;
    ...
    key: "_getFileNameForMoving",
    value: function _getFileNameForMoving(instance, tmp) {
      var fileName = nameFromUrl(tmp);

      if (!this._FOLDER_KEY || !this._GROUP_BY_ATTRIBUTE) {
        var _getFileInfo = getFileInfo(fileName);

        var _getFileInfo2 = _slicedToArray(_getFileInfo, 2);

        var name = _getFileInfo2[0];
        var ext = _getFileInfo2[1];
    ...
    value: function _moveFromTemporary(file, instance) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var instancePath = _this3._getInstancePath.bind(_this3)(instance);
        // const newPath = instancePath +
        //  `/${nameFromUrl(path)}` ;

        var newPath = _this3._getFileNameForMoving.bind(_this3)(instance, path);
  ...
  return new Promise(function (resolve, reject) {
    _request2.default.head(url, function (err, res, body) {
      ...
      if (res.statusCode < 200 || res.statusCode >= 400) {
        return reject("Can't download resource: \"" + url + "\" responded with " + ("\"" + res.statusCode + ": " + res.statusMessage + "\""));
      }
